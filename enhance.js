function addTabAndStoryEnhancements() {
  ...
  if (titleSub && !document.querySelector('#othersToggle')) {
  ...
  const othersToggle = document.querySelector('#othersToggle');
  const tabButtons = document.querySelectorAll('.profile-tabs button');
  const tabPanels = document.querySelectorAll('.tab-panel');

  const showPostsMode = () => {
    tabs.hidden = true;
    othersToggle?.classList.remove('is-open');
    document.body.classList.remove('is-others-mode');
    tabButtons.forEach((button) => button.classList.toggle('is-active', button.dataset.tab === 'posts'));
