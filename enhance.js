function addTabAndStoryEnhancements() {
  ...
  style.textContent = `
    ...
    .section-title{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr));align-items:center;gap:8px}.posts-toggle,.others-toggle,.notes-toggle,.tools-toggle{min-height:38px;border:1px solid rgba(168,85,247,.32);border-radius:999px;display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:0 10px;color:#fff;background:rgba(255,255,255,.065);cursor:pointer;font-size:.8rem;font-weight:900;box-shadow:inset 0 0 20px rgba(168,85,247,.05)}.posts-toggle:hover,.posts-toggle:active,.posts-toggle.is-open,.others-toggle:hover,.others-toggle:active,.others-toggle.is-open,.notes-toggle:hover,.notes-toggle:active,.notes-toggle.is-open,.tools-toggle:hover,.tools-toggle:active,.tools-toggle.is-open{background:rgba(168,85,247,.2);box-shadow:0 0 32px rgba(168,85,247,.42)}
    ...
    body.is-others-mode .tab-panel[data-panel="skills"],body.is-others-mode .tab-panel[data-panel="projects"],body.is-others-mode .tab-panel[data-panel="services"],body.is-others-mode .tab-panel[data-panel="notes"],body.is-others-mode .tab-panel[data-panel="tools"]{margin:10px;border:1px solid rgba(168,85,247,.18);border-radius:22px;background:rgba(255,255,255,.035)}body.is-others-mode .tab-panel[data-panel="projects"],body.is-others-mode .tab-panel[data-panel="services"],body.is-others-mode .tab-panel[data-panel="notes"],body.is-others-mode .tab-panel[data-panel="tools"]{margin-top:0}
    ...
    @media (min-width:720px){.section-title{max-width:720px;margin-inline:auto}.posts-toggle,.others-toggle,.notes-toggle,.tools-toggle{min-height:42px;font-size:.9rem}.notes-panel{max-width:720px;margin-inline:auto!important}}
  `;
  ...
  if (sectionTitle) {
    sectionTitle.innerHTML = `
      ...
      <button class="tools-toggle" id="toolsToggle" type="button"><i class="fa-solid fa-screwdriver-wrench"></i><span>Tools</span></button>
    `;
  }
  ...
  if (!document.querySelector('.tab-panel[data-panel="skills"]')) {
    ...
    panels.innerHTML = `
      ...
      <div class="tab-panel notes-panel tools-panel" data-panel="tools" hidden><div class="notes-head"><i class="fa-solid fa-screwdriver-wrench"></i><strong>Tools</strong></div><div class="notes-list" id="toolsList"><p>GitHub ke <strong>tools</strong> folder me files upload karo, yahan show honge.</p></div></div>
    `;
  ...
  const setButtons = (active) => {
    ...
    toolsToggle?.classList.toggle("is-open", active === "tools");
  ...
  const showToolsMode = () => { setButtons("tools"); setPanel(["tools"]); };
  ...
  toolsToggle?.addEventListener("click", showToolsMode);

  async function getGithubFolderFiles(folder) {
    ...
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${folder}`, { cache: "no-store" });
    if (!response.ok) return [];
    return (await response.json()).filter((file) => file.type === "file" && !file.name.startsWith("."));
  }
  ...
  function renderFolderList(list, files, emptyText, opensInViewer) {
    ...
    files.forEach((file) => {
      ...
      if (opensInViewer) {
        ...
        open.href = file.download_url;
        open.target = "_blank";
        open.rel = "noreferrer";
        open.innerHTML = '<i class="fa-solid fa-arrow-up-right-from-square"></i><span>Open</span>';
        actions.appendChild(open);
      }
  ...
  async function loadFolder(folder, listSelector, emptyText, opensInViewer) {
    const list = document.querySelector(listSelector);
    if (!list) return;
    const files = await getGithubFolderFiles(folder);
    renderFolderList(list, files, emptyText, opensInViewer);
  }
