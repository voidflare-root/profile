(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .info-section,.stories-section{display:none!important}.tab-panel[hidden]{display:none!important}.notes-list:empty{display:none!important}#noteClose,#noteCanvasClose{display:none!important}.section-title{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr));align-items:center;gap:8px}.notes-panel{padding:12px}.notes-list{display:grid;gap:10px}.note-item{min-height:74px;border:1px solid rgba(168,85,247,.24);border-radius:18px;display:grid;gap:10px;padding:12px}.note-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.note-actions a,.note-actions button{min-height:38px;border:1px solid rgba(168,85,247,.28);border-radius:14px;display:inline-flex;align-items:center;justify-content:center;gap:8px;color:#fff;background:rgba(255,255,255,.06);cursor:pointer;font-size:.82rem;font-weight:900;text-decoration:none}
  ...
  async function files(folder) {
    try {
      ...
      return (await response.json()).filter((file) => file.type === "file" && !file.name.startsWith(".") && file.download_url);
    } catch {
      return [];
    }
  }
  ...
  function renderFolder(folder, selector) {
    ...
    if (!list) return;
    list.innerHTML = "";
    files(folder).then((items) => {
      list.innerHTML = "";
      items.forEach((file) => {
        const item = document.createElement("article");
        item.className = "note-item";
        item.innerHTML = `<strong>${title(file.name)}</strong><div class="note-actions"><button type="button"><i class="fa-solid fa-eye"></i><span>Open</span></button><a href="${file.download_url}" download="${file.name}"><i class="fa-solid fa-download"></i><span>Download</span></a></div>`;
  ...
  function normalize() {
    ...
    activate(document.querySelector(".tab-panel.is-active")?.dataset.panel || "posts");
  }
  ...
  document.addEventListener("click", (event) => {
    ...
    activate(button.id === "postsToggle" ? "posts" : button.id === "othersToggle" ? "skills" : button.id === "notesToggle" ? "notes" : "tools");
  }, true);

  if (document.readyState === "loading") window.addEventListener("DOMContentLoaded", normalize); else normalize();
  window.addEventListener("load", () => { normalize(); setTimeout(ensureSkills, 500); setTimeout(ensureSkills, 1200); });
})();
